...
See the [Repository setup section in README](../README.md#repository-setup--github-packages) for full details.

In short:

1. Create `~/.m2/settings.xml` with your GitHub token:
```xml
<settings>
    <servers>
        <server>
...
> The `<id>` (`github-gargantua`) must match in both files. Projects generated by the archetype already have this configured.
