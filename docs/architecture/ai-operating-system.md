...
|---|---|---|
| Control Plane (Registry + Catalog + Policy + Deployment) | `gargantua-control-plane` | **MVP** — Spring Boot 4.1 / Java 21, publish→index→discovery funzionante, usa `agent-core` condiviso |
| Studio (frontend + BFF, un solo repo/immagine) | `gargantua-studio` | **MVP** — React Flow / Monaco / Zustand + backend Java 21/Boot 4.1 nello stesso repo (il vecchio `gargantua-studio-backend` è stato unito qui, 2026-08); costruisce manifest `gargantua.ai/v1` da form, gateway verso il Control Plane, multi-CP Settings |
