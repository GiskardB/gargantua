...
class ToolRegistryCompositeTest {
    ...
    /**
     * Registry over an empty Spring context, so nothing is contributed by annotation
     * scanning and only the supplied providers are under test.
     */
    private static ToolRegistry registryOver(List<ToolProvider> providers) {
        return new ToolRegistry(new StaticApplicationContext(),
                EmptyObjectProvider.instance(), EmptyObjectProvider.instance(),
                EmptyObjectProvider.instance(), EmptyObjectProvider.instance(),
                providers);
    }

    private static ToolDefinition tool(String name) {
        return new ToolDefinition(name, name + " description", false, false, false, "", new String[0], false, List.of());
    }

    private static ToolDefinition toolWithParameters(String name, ToolParameter... parameters) {
        return new ToolDefinition(name, name + " description", false, false, false, "",
                new String[0], false, List.of(parameters));
    }

    @Test
    @DisplayName("merges tools from every provider")
    void mergesToolsFromEveryProvider() {
        var registry = registryOver(List.of(
    ...
    void routesExecutionToOwningProvider() {
        ...
        var registry = registryOver(List.of(local, remote));
        registry.scan();
    ...
    void discoveryFailureIsIsolated() {
        ...
        var registry = registryOver(List.of(broken, healthy));
    ...
    void unknownToolReturnsError() {
        var registry = registryOver(List.of(new FakeProvider("annotation", List.of())));
        registry.scan();
    ...
    void nullContextTolerated() {
        ...
        var registry = registryOver(List.of(provider));
        registry.scan();
    ...
    void parameterTypesReachSpecification() {
        var registry = registryOver(List.of(new FakeProvider("mcp:api", List.of(
    ...
    void shutdownClosesProviders() {
        ...
        var registry = registryOver(List.of(first, second));
        registry.scan();
